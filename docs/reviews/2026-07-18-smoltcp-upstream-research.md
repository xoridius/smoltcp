...
> **Historical audit snapshot:** Findings below describe the pre-landing fork.
> Current disposition is recorded in `FORK.md` and
> `docs/perf/2026-07-18-after.md`.
...
| Default-checksum 6LoWPAN UDP is reconstructed with zero checksum and then rejected | src/iface/interface/sixlowpan.rs:788-810 ignores the NHC checksum and src/wire/udp.rs:280-287 writes zero; IPv6 UDP rejects zero at 249-260. Preserve inline checksums and reject elision unless the decompressor can identify and verify the additional integrity check required by RFC 6282 section 4.3.2; test fragmented/nonfragmented socket delivery with default capabilities. | Fork-specific interaction caused by the fork's correct IPv6 zero-checksum rejection. Not enabled by the medium-IP consumer. |
