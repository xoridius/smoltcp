...
impl TunTapInterface {
    ...
    ///
    /// The file descriptor is duplicated, so the caller retains ownership of `fd`.
    ...
    /// Attaches to a TUN/TAP interface, taking ownership of file descriptor `fd`.
    pub fn from_owned_fd(fd: OwnedFd, medium: Medium, mtu: usize) -> io::Result<TunTapInterface> {
        let lower = sys::TunTapInterfaceDesc::from_owned_fd(fd, mtu)?;
        Ok(TunTapInterface {
            lower: Rc::new(RefCell::new(lower)),
            mtu,
            medium,
